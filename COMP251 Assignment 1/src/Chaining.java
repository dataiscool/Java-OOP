...
public class Chaining {
    ...
    public int chain (int key) {	
        return ((this.A*key) % power2(this.w)) / power2(this.w-this.r);
    }
        
    
    /**Inserts key k into hash table. Returns the number of collisions encountered*/
    public int insertKey(int key){
    	// Get bucket (slot) associated with key
    	ArrayList<Integer> currBucket = this.Table.get(chain(key));
    	// First consider case where bucket is empty
    	if(currBucket.size() == 0) {
    		currBucket.add(key);
    		return 0;
    	}
    	// Now consider case where bucket not empty
    	else {
    		currBucket.add(key);
    		return currBucket.size() - 1 ; // Exclude the key we just added in count of collisions
    	}
    }
