...
class Edge{ // each edge defined by a node u, node v and weight w (u,v,w)
...
public class WGraph{
    ...
    WGraph() { // call to create empty graph from which to build
    }
    
    WGraph(WGraph graph) {
    	for(Edge e:graph.edges){
    		this.addEdge(new Edge(e.nodes[0],e.nodes[1],e.weight)); //this. refers to the new graph being copied over
    	}
